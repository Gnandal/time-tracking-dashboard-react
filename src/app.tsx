...
export function App() {
  ...
  const filterName = getActiveFilterName() as 'daily' | 'weekly' | 'monthly';
  ...
  return (
    <div className='flex flex-col lg:flex-row gap-8'>
      ...
      <div className='lg:w-3/4 w-full flex flex-col lg:grid lg:grid-cols-3 lg:grid-rows-2 gap-6'>
        {
          tasks.map((task: Task) => {
            return <TaskCard title={task.title} current={task.timeframes[filterName].current} previous={task.timeframes[filterName].previous}/>
          })
        }
      </div>
    </div>
